...
const SelectSeats = () => {
    ...
    const addDepSeatCallback = ({ row, number, id, cabin }, addCb) => {
        ...
        if(cabin === 'First'){
          ...
          depFirstSeats.allSeats[row.charCodeAt(0) - 65].map(
            (seat, i) => (i === (sId - 1) && seat) ? {id: user._id, number, row: row, isSelected: true, tooltip: 'Reserved by you'} : seat), 
    ...
    const addRetSeatCallback = ({ row, number, id, cabin }, addCb) => {
      ...
      if(cabin === 'First'){
        ...
        setRetFirstSeats({availableSeatsNum: retFirstSeats.availableSeatsNum - 1 , allSeats: [...retFirstSeats.allSeats.slice(0, row.charCodeAt(0) - 65)
          ...
          (seat, i) => (i === (sId - 1) && seat) ? {id: user._id, number, isSelected: true, row: row, tooltip: 'Reserved by you'} : seat), 
    ...
    const handleSubmit = () => {
        ...
        console.log(location.state.departureFlight, "1");
        console.log(location.state.returnFlight, "2");
        console.log(depSeatsOfPass, "3");
        console.log(retSeatsOfPass, "4");
        console.log(scData.depCriteria.cabin, "4");
        console.log();


        history.push(`/ReservedFlights`,{dFlight: location.state.departureFlight, rFlight: location.state.returnFlight, dSeats: depSeatsOfPass, rSeats:retSeatsOfPass,
